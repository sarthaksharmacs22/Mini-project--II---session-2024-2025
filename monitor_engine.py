...
# Create Flask app for monitor API
monitor_api = Flask(__name__)  # Changed variable name from 'api' to 'monitor_api'

@monitor_api.route('/get_activities')
def get_activities():
    return jsonify({
        "status": "success",
        "activities": suspicious_activities[-20:]  # Return last 20 events
    })

def run_monitor_api():
    monitor_api.run(port=5001)  # Now using monitor_api instead of api

if __name__ == "__main__":
    api_thread = threading.Thread(target=run_monitor_api, daemon=True)
    api_thread.start()
    monitor_exam()